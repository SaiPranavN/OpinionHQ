...
/**
 * The two things OpinionHQ actually does, side by side.
 *
    ...
    >
      <div className="mx-auto max-w-[1200px]">
        <div data-reveal className="ohq-reveal mx-auto max-w-[760px] text-center">
          <span className="ohq-eyebrow">Two ways to ask</span>
        ...
        <div className="mt-[clamp(38px,6vw,70px)] grid grid-cols-1 gap-[clamp(16px,2vw,24px)] lg:grid-cols-2">
          ...
          >
            <header className="flex flex-col gap-3">
              <span className="flex items-center gap-2.5 font-mono text-[10.5px] tracking-[0.14em] uppercase text-[#C4B5FD]">
                <span className="h-1.5 w-1.5 rounded-full bg-[#A78BFA]" />
                Polling
              </span>
              <h3 className="m-0 font-serif text-[clamp(1.7rem,2.6vw,2.4rem)] leading-[1.06] font-normal tracking-[-0.02em] text-cream-bright">
                Two options. <em className="italic">Pick one.</em>
              </h3>
              <p className="m-0 text-[14.5px] leading-[1.6] font-light text-muted">
                Head-to-head on anything — people, products, formats, policies, career
                paths. No neutral option and no fence to sit on, which is exactly why
                the result tells you something a sentiment scale cannot.
              </p>
            </header>
            ...
            <ul className="m-0 flex list-none flex-col gap-2 p-0 text-[13.5px] text-soft">
              {[
                "One bar, two sides, and the margin stated in plain words",
                "Cross-tabs by region, age and occupation — where the split flips",
                "Reasons in two columns. No replies, no thread, no pile-on",
              ].map((line) => (
                <li key={line} className="flex gap-2.5">
                  <span aria-hidden className="pt-px text-[#A78BFA]">
                    ✓
                  </span>
                  {line}
                </li>
            ...
            <footer className="mt-auto flex flex-wrap items-center gap-4 border-t border-line pt-5">
              <Link
                href="/polls"
                className="rounded-full bg-[#A78BFA] px-6 py-3 text-[14.5px] font-semibold text-[#1B1233] transition-[background,box-shadow] duration-500 ease-ohq hover:bg-[#B9A2FC] hover:shadow-[0_12px_36px_-10px_rgba(167,139,250,0.5)]"
              >
...
function SentimentMini() {
  ...
  return (
    <figure
      ...
      className="m-0 flex flex-col gap-3 rounded-[16px] border border-white/8 bg-surface-sunken p-5"
    >
  ...
  return (
    ...
    >
      <figcaption className="flex flex-wrap items-baseline justify-between gap-2">
        ...
        <span className="text-[15px] font-semibold text-[#C4B5FD]">53% JEE</span>
      </figcaption>
      <div className="flex h-9 gap-[3px]">
        ...
        <span
          className="flex origin-right animate-grow-x items-center justify-end rounded-[4px] pr-2.5 text-[13px] font-semibold text-[#1B1233]"
          style={{ width: "53%", background: "#A78BFA", animationDelay: "260ms" }}
        >
          53%
        </span>
      </div>
